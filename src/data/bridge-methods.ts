...
const definitions: readonly BridgeMethodDefinition[] = [
  ...
  ['VKWebAppTranslate', '翻译文本', 'basic-environment', '请求 VK 客户端翻译指定文本或文本列表。', 'low', 'vk-container', {texts: ["Приветкакдела?Былавозможностьпосмотретькнигу"],translation_language: "ru-en"}, true, false],
  ...
  ['VKWebAppGetAuthToken', '获取用户令牌', 'users-permissions', '请求用户授权并获取指定应用权限的令牌。', 'high', 'vk-container', { app_id: 54769936, scope: 'friends' }, true, true],
  ['VKWebAppGetCommunityToken', '获取社区令牌', 'users-permissions', '请求指定社区范围的访问令牌。', 'high', 'vk-container', { app_id: 54769936, group_id: 241534146, scope: 'messages' }, true, true],
  ['VKWebAppGetCommunityAuthToken', '获取社区授权令牌', 'users-permissions', '为应用获取社区授权令牌，可能触发权限确认。', 'high', 'vk-container', { group_id: 241534146, scope: 'messages' }, true, true],
  ...
  ['VKWebAppGetGroupInfo', '获取社区信息', 'users-permissions', '获取指定 VK 社区的公开信息。', 'low', 'vk-container', { group_id: 241534146 }, true, false],
  ['VKWebAppGetFriends', '获取好友列表', 'users-permissions', '获取当前用户允许小程序访问的好友列表。', 'medium', 'vk-container', { multi: false }, false, true],
  ['VKWebAppCallAPIMethod', '调用 VK API', 'users-permissions', '通过 VK Bridge 调用 VK API 方法，参数必须由开发者手动确认。', 'high', 'vk-container', { method: 'users.get', params: {user_ids: '743784474,743784479',v: '5.199',access_token: '通过 VKWebAppGetAuthToken 获得'} }, true, true],
  ...
  ['VKWebAppShowLeaderBoardBox', '展示排行榜弹窗', 'social-navigation', '打开 VK 客户端提供的排行榜界面并展示用户成绩。', 'medium', 'vk-container', { user_result: 100 }, true, true],
  ...
  ['VKWebAppOpenApp', '打开其他应用', 'social-navigation', '打开另一个 VK 应用，可能离开当前小程序。', 'high', 'vk-container', { app_id: 54769936 }, true, true],
  ['VKWebAppClose', '关闭小程序', 'social-navigation', '关闭当前小程序并向 VK 客户端报告状态。', 'high', 'vk-container', { status: 'success' }, true, true],
  ['VKWebAppJoinGroup', '加入社区', 'social-navigation', '请求用户加入指定 VK 社区。', 'high', 'vk-container', { group_id: 241534146 }, true, true],
  ['VKWebAppLeaveGroup', '退出社区', 'social-navigation', '请求用户退出指定 VK 社区。', 'high', 'vk-container', { group_id: 241534146 }, true, true],
  ['VKWebAppAllowMessagesFromGroup', '允许社区发消息', 'social-navigation', '请求用户允许指定社区向其发送消息。', 'high', 'vk-container', { group_id: 241534146 }, true, true],
  ...
  ['VKWebAppSendPayload', '发送社区消息载荷', 'social-navigation', '向指定社区发送自定义消息载荷。', 'high', 'vk-container', { group_id: 241534146, payload: {foo: 'bar'} }, true, true],
  ['VKWebAppSubscribeStoryApp', '订阅故事应用', 'social-navigation', '请求用户订阅当前应用的故事更新。', 'medium', 'vk-container', { app_id: 54769936 }, true, true],
  ['VKWebAppShowStoryBox', '打开故事发布框', 'social-navigation', '打开 VK 故事编辑和发布流程，故事内容必须由用户确认。', 'high', 'vk-container', {background_type: 'image',url : 'https://sun9-65.userapi.com/c850136/v850136098/1b77eb/0YK6suXkY24.jpg',attachment: {text: 'book',type: 'photo',owner_id: 743784474,id: 12345678}}, true, true],
  ['VKWebAppOpenWallPost', '打开墙面帖子', 'social-navigation', '打开指定用户或社区的墙面帖子。', 'low', 'vk-container', {owner_id: -166562603,post_id: 4604}, true, false],
  ...
  ['VKWebAppOpenPayForm', '打开支付表单', 'ads-payments-subscriptions', '打开支付表单，真实交易前必须由用户确认。', 'high', 'vk-container', {app_id: 54769936,action: 'pay-to-user',params: {user_id: 1124078883}}, true, true],
  ['VKWebAppShowOrderBox', '打开订单框', 'ads-payments-subscriptions', '打开订单购买流程，可能产生真实支付。', 'high', 'vk-container', { type: 'item',item: 'item_id_123456' }, true, true],
  ['VKWebAppShowSubscriptionBox', '打开订阅框', 'ads-payments-subscriptions', '打开订阅流程，可能产生持续性扣款。', 'high', 'vk-container', { action: 'create', item: '25' }, true, true],
  ...
  ['VKWebAppShowSlidesSheet', '展示幻灯片面板', 'ads-payments-subscriptions', '展示由 VK 客户端提供的幻灯片式内容面板。', 'medium', 'vk-container', {slides: [{media: {blob: 'data:image/png;base64,[IMAGE_DATA]',type: 'image'},title: 'Slide title',subtitle: 'Slide description under the title'}]}, true, true],
  ['VKWebAppShowCommunityWidgetPreviewBox', '预览社区组件', 'ads-payments-subscriptions', '预览指定社区组件，内容和社区信息需手动确认。', 'high', 'vk-container', { group_id: 241534146, type: 'text', code: 'return {\"title\": \"Quote\",\"text\": \"Quote text\"}' }, true, true],
  ...
  ['VKWebAppRetargetingPixel', '发送再营销事件', 'analytics', '向 VK 发送再营销像素事件。', 'high', 'vk-container', { pixel_code: 'VK-RTRG-447253-dUuM', event: 'click-to-button' }, true, false],
  ['VKWebAppConversionHit', '发送转化事件', 'analytics', '向 VK 发送转化命中事件。', 'high', 'vk-container', {pixel_code: 'VK-Boo-427253-dUuM', conversion_event: 'purchase',conversion_value: 3500}, true, false],
  ['VKWebAppTrackEvent', '记录统计事件', 'analytics', '记录小程序的业务统计事件。', 'medium', 'vk-container', {event_name: 'game_event',custom_user_id: '743784479',event_params: {level: 5,mode: 123}}, true, false],
] as const;
