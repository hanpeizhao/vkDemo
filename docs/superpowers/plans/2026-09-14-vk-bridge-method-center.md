...
**Architecture:** 使用独立的官方方法注册表驱动分类和页面；使用纯函数负责 JSON 参数解析、敏感字段脱敏、方法调用超时和结果归一化；使用受类型约束的 Bridge 适配层调用 VK Bridge。Persik 只负责展示和交互，既有能力实验室页面继续保持可用。
...
- [ ] 保留现有能力实验室需要的数据导出或兼容结构，避免破坏已有分类页。
