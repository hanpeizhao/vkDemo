...
首页提供基础信息、VK Bridge、VKUI 组件、布局与导航以及调试日志分类。Bridge 测试需要从 VK 小程序容器中打开；在普通浏览器中运行时，页面会显示环境提示，不会一直停留在加载状态。
...
- 调试日志只保存执行状态、耗时和脱敏结果。令牌、密钥、哈希、手机号、邮箱等敏感字段会显示为“[已脱敏]”，不要把真实凭据或个人信息粘贴到参数框中。
